...
//    float x1=-0.15,y1=0.25,z1=0.15;
//    float x2=0.05,y2=0.3,z2=0.15;
//    float x3=0.2,y3=0.3,z3=0.15;
//    object1.setPosition(x1,y1,z1);
//    object2.setPosition(x2,y2,z2);
//    object3.setPosition(x3,y3,z3);

    object1.setPosition(target1.x1,target1.y1,0.0375);
    object2.setPosition(target1.x2,target1.y2,0.0375);
    object3.setPosition(target1.x3,target1.y3,0.0375);
...
//    float target_x1 = 0.0, target_y1 = 0.2, target_z1 = 0.2;
//    float target_x2 = -0.1, target_y2 = 0.2, target_z2 = 0.2;
//    float target_x3 = 0.1, target_y3 = 0.2, target_z3 = 0.2;
//
//    place_pose1.setTargetPose(target_x1, target_y1, target_z1, 0, 1, 0, 0);
//    place_pose2.setTargetPose(target_x2, target_y2, target_z2, 0, 1, 0, 0);
//    place_pose3.setTargetPose(target_x3, target_y3, target_z3, 0, 1, 0, 0);
    // 调用类中的阻塞方法等待消息
    node->waitForMessage();

    // 获取接收到的目标位置信息并赋值给 target1
    node->getPosition(target2);
    // 将 order1, order2, order3 放入队列
    order_queue.push(target2.order1);
    order_queue.push(target2.order2);
    order_queue.push(target2.order3);

    place_pose1.setTargetPose(target2.x1, target2.y1, 0.2, 0, 1, 0, 0);
    place_pose2.setTargetPose(target2.x2, target2.y2, 0.2, 0, 1, 0, 0);
    place_pose3.setTargetPose(target2.x3, target2.y3, 0.2, 0, 1, 0, 0);

    // 根据队列中的顺序依次执行操作
    while (!order_queue.empty()) {
        int current_order = order_queue.front();  // 获取队列的第一个元素
        order_queue.pop();  // 移除第一个元素

        // 使用 switch 根据顺序执行相应的任务
        switch (current_order) {
            ...
            case 3:
                ...
                break;
            default:
                RCLCPP_WARN(node->get_logger(), "Unknown order %d", current_order);
                break;
        }
    }
