...
Trajectory::Trajectory(rclcpp::Node::SharedPtr node, const std::string& yaml_file) {
    // 直接在构造函数中初始化发布器
    trajectory_pub_ = node->create_publisher<moveit_msgs::msg::RobotTrajectory>("manipulator_trajectory", 10);
...
bool Trajectory::executeTrajectory(moveit::planning_interface::MoveGroupInterface& move_group,
    ...
    // 轨迹执行成功后，发布轨迹
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Trajectory executed successfully, publishing trajectory.");
    trajectory_pub_->publish(msg_trajectory);
...
//     plan.trajectory_.joint_trajectory.points = msg_trajectory.joint_trajectory.points;
//
//     const int max_retries = 101;
//     int retries = 0;
//     moveit::core::MoveItErrorCode execution_result = moveit::core::MoveItErrorCode::FAILURE;
//
//     bool collision_check_disabled = false;
//
//     while (retries < max_retries) {
//         execution_result = move_group.execute(plan);
//
//         if (execution_result == moveit::core::MoveItErrorCode::SUCCESS) {
//             return true;
//         }
//
//         // 第100次失败时禁用碰撞检查
//         if (retries == 99 && !collision_check_disabled) {
//             move_group.allowReplanning(false);  // 禁用重规划（避免碰撞检测）
//             collision_check_disabled = true;
//             RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Disabling collision checking on retry 100.");
//         }
//
//         retries++;
//         RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Execution failed. Retrying... (%d/%d)", retries, max_retries);
//     }
//
//     // 恢复碰撞检查
//     if (collision_check_disabled) {
//         move_group.allowReplanning(true);  // 恢复重规划
//         RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Collision checking has been restored.");
//     }
//
//     RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Execution failed after %d retries.", max_retries);
//     return false;
// }
