...
// 订阅 "轨迹名字+开始执行" 和 "执行完成" 消息
void trajectory_start_callback(const std_msgs::msg::String::SharedPtr msg) {
    // 当收到 "轨迹名字+开始执行" 消息时
    is_executing_trajectory = true;
    RCLCPP_INFO(rclcpp::get_logger("TrajectoryControl"), "开始执行轨迹: %s", msg->data.c_str());

    // 构造文件路径并读取 YAML 文件
    std::string file_path = "../config/" + msg->data + ".yaml";
    read_trajectory_from_yaml(file_path);  // 读取目标轨迹文件
}

void trajectory_complete_callback(const std_msgs::msg::String::SharedPtr msg) {
    // 当收到 "执行完成" 消息时
